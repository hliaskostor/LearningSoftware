...
{
    public partial class beginnerList : Form
    {
        int currentImageIndex = 0;
        List<Image> images = new List<Image>();
        string logUser;
        public beginnerList(string username)
        {
            InitializeComponent();
            logUser = username;
        }

        private void beginnerList_Load(object sender, EventArgs e)
        {
            LoadImages();
        }
        ...
        {
            QuizBegList blst=new QuizBegList(logUser);
            this.Hide();
            blst.Show();
